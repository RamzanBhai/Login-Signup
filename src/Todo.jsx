...
const Todo = () => {
  ...
  return (
    <div className='flex items-center justify-center w-full h-screen  bg-gray-600'>
      <div className='bg-white p-5 rounded-lg h-80  ' >
      <h1 className='text-xl font-extrabold mb-3'>Todo </h1> 
      <div className=' mb-3'>
         <input placeholder='Enter your Tasks' className='outline-none border-2 p-1 rounded-lg mr-3 text-xl font-bold' type="text" value={inputValues} onChange={(e)=>dispatch(setInput((e.target.value)))}/>
     ...
     <div>
      ...
      <div className='p-1 rounded-lg overflow-y-scroll h-42 scrolls'>
      {setValues.map((item,index)=>(
       ...
       <Link to='/' className=' fixed bottom-42 right-135 bg-gray-600  text-white font-bold rounded-lg p-2'>Home</Link>
     </div>
     </div>
    </div>
